...
def user_stats_channels_join(u_id :int)->None:
...
def user_stats_channels_leave(u_id :int)->None:
    store = data_store.get()
    channels = store['user_stats'][u_id]['channels_joined']
    num_channels = channels[-1]['num_channels_joined'] - 1
...
def user_stats_dms_join(u_id :int)->None:
    store = data_store.get()
...
def user_stats_dms_leave(u_id :int)->None:
    store = data_store.get()
    dms = store['user_stats'][u_id]['dms_joined']
    num_dms = dms[-1]['num_dms_joined'] - 1
...
def user_stats_messages(u_id :int)->None:
    store = data_store.get()
